    ...
    public void onCommand(final CommandSender sender, final String[] args, final String prefix) {
        ...
        if (map.getPrivacyLevel() == SketchMap.PrivacyLevel.PRIVATE
            ...
            player.sendMessage(
                    ChatColor.RED
                            + prefix
                            + "You don't have permission to place map "
                            + map.getID()
                            + ".");
            return;
        }
        ...
        if (targetBlock.getType() == Material.AIR) {
            player.sendMessage(
                    ChatColor.RED
                            + prefix
                            + "Could not find target block. Ensure you are looking"
        ...
        for (final RelativeLocation relLoc : mapCollection.keySet()) {
            ...
            if (loc.getBlock().getType() != Material.AIR
                    || backLoc.getBlock().getType() == Material.AIR) {
                player.sendMessage(
                        ChatColor.RED
                                + prefix
                                + "There is not enough room on that wall to place that Sketch Map");
            ...
            try {
                ...
                // iMeta.setDisplayName(ChatColor.GREEN + "SketchMap ID: " + ChatColor.GOLD +
                // map.getID() +
                ...
                //    iFrames.remove(itemFrame);
                player.sendMessage(
                        ChatColor.RED + prefix + "Unable to place image on that surface.");
                return;
            }
        }
        player.sendMessage(
