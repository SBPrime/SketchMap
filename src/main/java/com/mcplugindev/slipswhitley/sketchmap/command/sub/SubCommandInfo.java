        ...
        if (sender instanceof Player) {
            ...
            if (!map.getOwnerUUID().equals(player.getUniqueId()))
                if (!SketchMapUtils.hasPermission(player, "sketchmap.privacy.admin"))
                    return; // Don't display list of allowed players to players that aren't the
                            // owner or who lack sketchmap.list.admin
        }

        // List will display to console, the sketchmap's owner, and players with
        // sketchmap.list.admin
        if (map.getPrivacyLevel() == SketchMap.PrivacyLevel.PRIVATE)
            sender.sendMessage(
                    ChatColor.GREEN + "Allowed players: " + ChatColor.AQUA + allowedNames);
        else sender.sendMessage(ChatColor.GREEN + "This map is publicly available.");
    }
}
