    ...
    public boolean onCommand(
        ...
        if (args.length == 0) {
            sender.sendMessage(
                    ChatColor.RED
                            + prefix
                            + "Command Arguments required. Try, \"/sketchmap help\"");
            return true;
        }
        for (final SketchMapSubCommand command : SketchMapSubCommand.getCommands()) {
            ...
            if (command.getPermission() != null
                ...
                sender.sendMessage(
                        ChatColor.RED + prefix + "You do not have permission to use this command.");
                return true;
            }
            ...
            if (args.length > 1) {
                String[] newArgs =
                        Arrays.copyOfRange(
                                args, 1, args.length); // Strip the subcommand off the front
                command.onCommand(sender, newArgs, prefix);
            } else {
                command.onCommand(
                        sender,
                        new String[] {},
                        prefix); // Sending no args if the only arg was the subcommand
            }
            return true;
        }
