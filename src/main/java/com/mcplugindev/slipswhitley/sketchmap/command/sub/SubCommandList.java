    ...
    public void onCommand(final CommandSender sender, final String[] args, final String prefix) {
        ...
        if (args.length == 0) {
            if (sender instanceof Player) {
                ...
                for (final SketchMap map : SketchMap.getLoadedMaps()) {
                    if ((map.getPrivacyLevel() == SketchMap.PrivacyLevel.PRIVATE
                        ...
                        continue;
                    if (map.isPublicProtected()) continue;
                    maps.add(map.getID());
                }
            } else {
                for (final SketchMap map : SketchMap.getLoadedMaps()) {
            ...
            if (pageNumber) {
                if (sender instanceof Player) {
                    ...
                    for (final SketchMap map : SketchMap.getLoadedMaps()) {
                        if ((map.getPrivacyLevel() == SketchMap.PrivacyLevel.PRIVATE
                                && !map.getOwnerUUID().equals(player.getUniqueId())
                                && !map.getAllowedUUID().contains(player.getUniqueId())
                                && !SketchMapUtils.hasPermission(
                                        player, "sketchmap.privacy.admin"))) continue;
                        if (map.isPublicProtected()) continue;
                        maps.add(map.getID());
                    }
                } else {
                ...
                if (listPlayer == null) {
                    sender.sendMessage(
                            ChatColor.RED + prefix + "Couldn't find player '" + args[0] + "'");
                    return;
                }

                if (sender instanceof Player
                        && !listPlayer.getUniqueId().equals(((Player) sender).getUniqueId())
                        && !SketchMapUtils.hasPermission(
                                ((Player) sender), "sketchmap.privacy.admin")) {
                    sender.sendMessage(
                            ChatColor.RED + prefix + "You don't have permission to do that");
                    return;
                }

                for (final SketchMap map : SketchMap.getLoadedMaps()) {
                    if (!map.getOwnerUUID().equals(listPlayer.getUniqueId())) continue;
                    if (map.isPublicProtected()) continue;
                    maps.add(map.getID());
                }
            }
        ...
        int numPages =
                (maps.size() / pageSize)
                        + (maps.size() % pageSize == 0
                                ? 0
                                : 1); // This works the same as array length (last page number is
                                      // one less than number of pages, yay zero!)

        int pageToShow;
        if (args.length == 0) pageToShow = 0;
        else if (args.length == 1) {
            ...
            if (pageNum) pageToShow = Integer.parseInt(args[0]) - 1;
            else pageToShow = 0;
        } else {
            try {
                ...
                sender.sendMessage(
                        ChatColor.RED
                                ...
                                + "'"
                                + args[1]
                                + "' is not a page number. Try again.");
                return;
            }
        }

        if (pageToShow >= numPages) pageToShow = numPages - 1;
        if (pageToShow < 0) pageToShow = 0;

        int pageStart = pageSize * pageToShow;
        int pageEnd = pageSize * (pageToShow + 1) - 1;
        if (pageEnd > maps.size()) pageEnd = maps.size() - 1;
        ...
        if (pageToShow + 1 < numPages)
            sender.sendMessage(
                    ChatColor.RED
                            ...
                            + ChatColor.BLUE
                            + "/sketchmap list "
                            + (args.length != 0 && args[0].equalsIgnoreCase("mine") ? "mine " : "")
                            + (pageToShow + 2)
                            + ChatColor.RED
