...
public class FileManager {
    ...
    public FileManager(final SketchMap sketchMap) {
        ...
        this.mapFile =
                new File(
                        SketchMapLoader.getMapsDirectory()
                                + "/"
                                + sketchMap.getID()
                                + ".sketchmap");
        if (!this.mapFile.exists()) {
            try {
                ...
                Bukkit.getLogger()
                        .log(
                                Level.WARNING,
                                "[SketchMap] Unable to create/load SketchMap file \""
                                        + this.mapFile.getName()
                                        + "\" in SketchMaps folder.",
                                ex);
                return;
            }
        }
        try {
            ...
            Bukkit.getLogger()
                    .log(
                            Level.WARNING,
                            "[SketchMap] Unable to load SketchMap file \""
                                    + this.mapFile.getName()
                                    + "\" in SketchMaps folder.",
                            ex);
        }
    ...
    public void save() {
        ...
        this.mapConfig.set(
                "map-image",
                SketchMapUtils.imgToBase64String(
                        this.sketchMap.getImage(), SketchMap.BaseFormat.PNG.getExtension()));
        try {
            ...
            Bukkit.getLogger()
                    .log(
                            Level.WARNING,
                            "[SketchMap] Unable to save SketchMap file \""
                                    + this.mapFile.getName()
                                    + "\" in SketchMaps folder.",
                            e);
        }
