...
public class RelativeLocation {
    ...
    public static RelativeLocation fromStringZeroBase(final String str) {
        ...
        try {
            ...
            if (x < 0 || y < 0) throw new Exception("negative coordinates are disallowed");
        } catch (Exception ex) {
            return null;
        }
    ...
    /**
     * Compares two RelativeLocation objects to see if they point to the same relative coordinates.
     * Does NOT check whether the coordinates are on the same sketchmap.
     *
     * @param otherLoc RelativeLocation object to be compared
     * @return true if the relative X and Y coordinates are the same for both objects, false
     *     otherwise
     */
