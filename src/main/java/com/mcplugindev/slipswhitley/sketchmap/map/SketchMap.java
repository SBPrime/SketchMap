...
public class SketchMap {
    ...
    public SketchMap(
            ...
            final int xPanes,
            final int yPanes,
            final boolean publicProtected,
            final BaseFormat format) {
    ...
    public SketchMap(
            ...
            final UUID ownerUUID,
            final PrivacyLevel privacyLevel,
            final List<UUID> allowedUUID,
            final int xPanes,
            final int yPanes,
            final boolean publicProtected,
            final BaseFormat format,
            final Map<Short, RelativeLocation> mapCollection) {
        this.image = SketchMapUtils.resize(image, xPanes * 128, yPanes * 128);
        this.mapID = mapID;
        this.ownerUUID = ownerUUID;
        this.privacyLevel = privacyLevel;
    ...
    public enum BaseFormat {
        ...
        public static BaseFormat fromExtension(final String ext) {
            if (ext.equalsIgnoreCase("png")) return BaseFormat.PNG;
            if (ext.equalsIgnoreCase("jpg") || ext.equalsIgnoreCase("peg")) return BaseFormat.JPEG;
            return null;
        }

        public String getExtension() {
            if (this == BaseFormat.PNG) return "png";
            if (this == BaseFormat.JPEG) return "jpg";
            return null;
        }
    }
