...
public class SketchMapAPI {
            ...
            for (int x = 0; x < map.getLengthX(); ++x) {
                for (final RelativeLocation loc : map.getMapCollection().keySet()) {
                    if (loc.getX() == x) {
                        ...
                        iStack.setDurability(
                                SketchMapUtils.getMapID(map.getMapCollection().get(loc)));
    ...
    public static SketchMap loadSketchMapFromFile(final File file) throws SketchMapFileException {
        ...
        try {
            ...
            if (image == null) throw new Exception("Make the real error message appear");
        } catch (Exception ex4) {
            throw new SketchMapFileException(
                    "Unable to load SketchMap file \""
                            + file.getName()
                            + "\"; cannot parse image from field \"map-image\"");
        }
